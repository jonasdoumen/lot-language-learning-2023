...
eng_ws$production |>
  mean()
```
The pipe takes the function name and puts it in the back, this is handy if we want to apply multiple fns, they'll get tricky to read
...
```
the eng_ws variable is not overwritten, if you wanted to do that, you say 
...
# ...
ggplot(eng_ws_sex, 
       aes(x = age, y = production, color = sex)) + 
  geom_point()
```
...
# ...
ggplot(eng_ws_sex, 
       aes(x = age, y = production, color = sex, size = production)) + xlab("age") + ylab("production") + geom_point() + xlim(0,max(eng_ws_sex$age)) + ylim(0,max(eng_ws_sex$production))
```
