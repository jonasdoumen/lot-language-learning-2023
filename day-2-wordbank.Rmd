...
```{r}
admins_dutch_ws <- get_administration_data(language = "Dutch", form = "WS", include_demographic_info = TRUE)
n_distinct(admins_dutch_ws$data_id)
admins_dutch_ws |> 
  count(age)
# ...
```
...
```{r}
ggplot(admins_dutch_ws, aes(x = age, y = production)) +
...
```{r}
items_dutch_ws |> distinct(item_kind)
```
...
ggplot(dog_cat_means, aes(x = age, y = produces, col = item_definition)) +
  geom_point() + 
...
```{r}
cor.test(admins_eng_ws$production^2, admins_eng_ws$complexity_score)
```
...
```{r}
items_eng_ws <- get_item_data(language = "English (American)", form = "WS")
pred_items <- filter(items_eng_ws, lexical_category == "predicates")
head(pred_items)
pred_instrument_data <- get_instrument_data(language = "English (American)", 
                                                  form = "WS", 
                                                  items = pred_items$item_id)

pred_means <- pred_instrument_data |>
  group_by(data_id) |>
  summarise(predicate_score = sum(value == "produces"))
head(pred_means)
admins_eng_ws <- get_administration_data(language = "English (American)", form = "WS")
head(admins_eng_ws)
admins_eng_ws <- left_join(admins_eng_ws, pred_means)
ggplot(admins_eng_ws, aes(x = production, y = predicate_score)) + 
  geom_jitter(alpha = .1, size = .5, width = 0, height = .3) + 
  geom_smooth()
```
