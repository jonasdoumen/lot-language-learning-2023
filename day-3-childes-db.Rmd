...
age of first sg dog vs age of first plural dog
=> you want to show a graph with all the points where dog is produced, so x-axis is age, y-axis is jitter?

Hint: `filter`, `group_by`, `summarise`!

```{r}
# dogs ->
#sg_dogs <- dogs |> filter(gloss =="dog")
#pl_dogs <- dogs |> filter(gloss =="dogs")
...
left_join(sg_dogs, pl_dogs) |>
  mutate(delay = min_age_pl - min_age_sg)
#
dogs |>
  filter(gloss %in% c ("dog", "dogs")) |>
  group_by(target_child_name, gloss) |>
  summarise(min_age = min(target_child_age))
```
...
EXERCISE: use `get_tokens` to 1) get all instances of "and" and "or" spoken by either the child or the child's mother in the Brown corpus and 2) in a separate dataframe get ALL the tokens spoken by the child and the mother (you'll use this to normalize the counts so as to be able to compare transcripts of different sizes). proportion_or: count_or/count_all_tokens

```{r}

and_or_tokens <- get_tokens(corpus = "Brown", role = c("target_child", "Mother"), token = c("and", "or"))
all_tokens <- get_tokens(corpus = "Brown", role = c("target_child", "Mother"), token = "*")
```
...
and_or_counts <- and_or_tokens |>
  group_by(speaker_id, transcript_id, speaker_role, target_child_age) |>
  summarise(n_and_or = n())
